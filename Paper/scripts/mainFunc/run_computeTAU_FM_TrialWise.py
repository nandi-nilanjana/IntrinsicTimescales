...
This function uses the subfunction mainFun_computeTAU_TrialWise defined in folder Paper/scripts/procMethods
...
def run_computeTAU_FM(inFolder,outFolder,lag_limit,eventMarker,excludeFirstBin):
  # Create folder if it does not exist
    ...
    for i, file_name in enumerate(file_names, start=1):
        ...
        if eventMarker == 'preSC2': #this is just based on how i sa
            trial_info = data[l[2]]
        else:
        ...
        if eventMarker  == 'preSC1' :
            ...
            t_st= data[l[1]]-2000 #start time to cut the trial that is 2 sec before SC1 onset; keys -  1 - Sc1 and 2 - Go_time
            t_end= data[l[1]] # at SC1 onset; stop time to cut the trial 
            trial_exclude = 0 #include all trials
        
        elif eventMarker == 'preGo':
            #depends on what is stored in the mat data file 
            t_st= data[l[2]]-2000 #start time to cut the trial that is 2 sec before GoTim onset ; keys -  1 - Sc1 and 2 - Go_time
            t_end= data[l[2]] # at GoTim onset; stop time to cut the trial 
            trial_exclude = 3  #excluding pink trials since 1 sec before the Go it receives the direction information , so to remove any variability from that
       
        elif eventMarker == 'preSC2': #ignore this for now . this was a particular check i did for only green trials
            t_st= data[l[1]]-1000
            t_end= data[l[1]]
            trial_include = 1
        ...
        if extracted_info.isdigit() :
            ...
            # Run main function
        [out_fit, out_autoc] = mainFun_computeTAU(d,unit,lag_limit,trial_exclude,t_st,t_end,trial_info,excludeFirstBin)    
...
if __name__=='__main__':
    ...
    if server == '/envau':
        sys.path.append('/envau/work/comco/nandi.n/IntrinsicTimescales/Paper/scripts/procMethods')
    else:
        sys.path.append('/Users/nilanjana/Documents/LabWorkBench/IntrinsicTimescales/Paper/scripts/procMethods')
    from computeTAU_FontanierMethod_TrialWise import mainFun_computeTAU
    from open_matlabData import open_matlab_behaviour
       
    #initialise variables
    lag_limit =[1000]
    eventM = ['preSC1', 'preGo'] #can be preSC1 or preGo depending on the bhv data extracted from matlab     
    excludeFirstBin = 1;
            ...
            for l in lag_limit:
                if monkey == 'Tomy':
                    session_names =['t140924003','t140924006','t140925001','t140926002','t140930001','t141001001','t141008001',
                                    ...
                                    't150430002','t150520003','t150716001','TomyAlmap2023']              
                ...
                for i_session in session_names:
                    ...
                    outFolder = f"{server}/work/comco/nandi.n/IntrinsicTimescales/Paper/data/SUA/{monkey}/{session}/TAU_{eventMarker}_{binExt}"
                    run_computeTAU_FM(inFolder,outFolder,l,eventMarker,excludeFirstBin)
            
        print(f'Success computing all fits: M {monkey}')
    print('Completed extracted for both monkeys')
