...
Updated on 28 Sept,2023
 ...
 Modified version from computeTAU_FontanierMethod;  not smoothening with loess as the main method beasue on such small number of intervals it throws error
 It alows arguments to take time stamps to cut the trial and trial type, and exclude a trial type to compute the ISIs
 Minor changes have been made to subfunction 1 and dip_func that has another variablw now called findfit
...
def compute_autoc(data,lag_limit,trial_exclude,t_st,t_end,trial_info) :
    # Specify the maximum lag order
    diff_order = 25  # Change this to the desired maximum lag order    
    trial =0; #index initialisation
    ISI_pertrial = []        
    # Iterate through trials and align spike data
    for trial_start, trial_end in zip(t_st, t_end):         
        if trial_info[trial] != trial_exclude: # for preGo the pink trials will be excluded
       # if  trial_info[trial] == trial_exclude: #just commenting here to extract only my desired trials - modified here to extract only blue trials in delay period 2 and also on green trials in delay period 2 to compare the tau in these two periods
    ...
    curr_autoc ['countsnsm']  = (curr_autoc['counts'] / curr_autoc_tot) * (1/wd) #here it is the same as countsn since no smoothening is done. But keeping the labels
    #same as so that I can use the rest of the subfunction as computeTAU_FontanierMethod.py
    ...
    if not(df_peak['peak1_lat']=='NA'):
        ...
        # if index is at the end of curr_begin it will throw an error - so this will also lead to no dip
                    ...
                    if ( min_idx < len(curr_begin['countsnsm']) and
                        ...
                        dip = curr_max - loc_min >= 3/4 * (curr_max-curr_min) #only considered as the second dip if the local minima is greater than equal to 75% of global max-min
                    else:
                        dip = False
...
def mainFun_computeTAU(data,unit,lag_limit,trial_exclude,t_st,t_end,trialType,excludeFirstBinPeak):
    print('Inside main function to compute TAU')

    curr_autoc,findfit = compute_autoc(data,lag_limit,trial_exclude,t_st,t_end,trialType)
